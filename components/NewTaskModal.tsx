...
import { currentUserMock, UserAvatar } from '../lib/utils';
...
export const NewTaskModal = ({ isOpen, onClose, projeto = null, taskToEdit = null }: NewTaskModalProps) => {
  ...
  const [formData, setFormData] = useState({
    titulo: '',
    descricao: '',
    projetoIds: projeto ? [projeto.id] : [],
    responsavelIds: [currentUserMock.id],
    prioridade: 'média',
    ...
    if (isOpen) {
      if (taskToEdit) {
        ...
        setFormData({
          ...
          projetoIds: taskToEdit.projetoId ? [taskToEdit.projetoId] : [],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload: any = {
      ...
      project_id: formData.projetoIds[0],
      assignee_ids: formData.responsavelIds,
    ...
    try {
      if (taskToEdit) {
        const { data, error } = await supabase
          ...
          .eq('id', taskToEdit.id)
          .select();
        if (error || !data || (Array.isArray(data) && data.length === 0)) throw error || new Error('Nenhuma linha atualizada');
      } else {
        const { data, error } = await supabase
          .from('tasks')
          .insert([payload])
  ...
  const handleSelectProject = (projectId: number) => {
    setFormData(prev => {
      ...
      if (exists) {
        return { ...prev, projetoIds: prev.projetoIds.filter(id => id !== projectId) };
      } else {
        return { ...prev, projetoIds: [...prev.projetoIds, projectId] };
      }
    });
  };
  ...
  const filteredProjects = projects.filter((p: any) =>
    p.nome.toLowerCase().includes(projectSearchQuery.toLowerCase())
  );

  const selectedProjects = projects.filter((p: any) =>
    formData.projetoIds.includes(p.id)
  );
    ...
    <div className="fixed inset-0 bg-gray-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4 transition-opacity">
      <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-gray-100">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          {/* Projetos Vinculados (Full Width) */}
          <div className="space-y-1.5 relative" ref={projectDropdownRef}>
            <label className="block text-sm font-semibold text-gray-700">
              Projetos Vinculados <span className="text-red-500">*</span>
            </label>
            <div className="min-h-[46px] w-full px-2 py-2 bg-gray-50 border border-gray-200 rounded-xl focus-within:bg-white focus-within:ring-2 focus-within:ring-blue-100 focus-within:border-blue-500 transition-all flex flex-wrap gap-2">
              {selectedProjects.map((p: any) => (
                  <span key={p.id} className="inline-flex items-center gap-1.5 px-3 py-1 bg-white border border-purple-100 text-purple-700 rounded-lg text-sm font-medium shadow-sm">
                  <Briefcase className="w-3.5 h-3.5" />
                  <span className="truncate max-w-[100px]">{p.nome}</span>
                  <button 
                    type="button"
                    onClick={() => handleSelectProject(p.id)}
                    className="hover:bg-purple-50 rounded-full p-0.5 transition-colors"
                  >
                    <X className="w-3 h-3" />
                  </button>
                </span>
              ...
              <div className="flex-1 relative min-w-[120px]">
                <input
                  ...
                  placeholder={selectedProjects.length === 0 ? "Buscar projeto..." : "Adicionar mais..."}
                />
              </div>
            </div>
            {showProjectDropdown && (
                ...
                {filteredProjects.length > 0 ? (
                  <div className="py-2">
                    {filteredProjects.map((p: any) => {
                      const isSelected = formData.projetoIds.includes(p.id);
                      return (
                        <button
                          key={p.id}
                          type="button"
                          onClick={() => handleSelectProject(p.id)}
                          className={`w-full px-4 py-2.5 flex items-center justify-between hover:bg-gray-50 transition-colors text-left ${isSelected ? 'bg-purple-50/50' : ''}`}
                        >
