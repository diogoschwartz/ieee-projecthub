...
export const ChapterChairPage = () => {
  ...
  // Inicializa com o primeiro capítulo disponível
  useEffect(() => {
  ...
  useEffect(() => {
    if (currentChapter) {
      setFormData({
        description: currentChapter.description || currentChapter.descricao || '',
        email: currentChapter.email || '',
        color_theme: currentChapter.color_theme || currentChapter.cor || 'from-blue-600 to-indigo-600',
        icon_name: currentChapter.icon_name || Object.keys(iconMap).find(key => iconMap[key] === currentChapter.icon) || 'FolderKanban',
        cover_image_url: currentChapter.cover_image_url || currentChapter.coverImage || '',
        calendar_url: currentChapter.calendar_url || (currentChapter as any).calendarUrl || '',
        ...
        <div className="relative min-w-[250px]">
          ...
          >
            {availableChapters.map((c: any) => (
              <option key={c.id} value={c.id}>{c.acronym || c.sigla} - {c.name || c.nome}</option>
            ))}
          </select>
        ...
        <div className="space-y-8">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between relative group/card">
            <div className="flex items-center gap-4">
              <div className={`w-14 h-14 rounded-xl bg-gradient-to-br ${currentChapter.color_theme || currentChapter.cor || 'from-blue-600 to-indigo-600'} flex items-center justify-center text-white shadow-md transition-all duration-300`}>
                <Users className="w-7 h-7" />
              </div>
              <div>
                <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2">
                  {currentChapter.name || currentChapter.nome}
                </h2>
              ...
              <div className="flex items-center gap-2 overflow-x-auto pb-2 md:pb-0">
                {availablePeriods.length > 0 && availablePeriods.map(period => (
                  <button
                    ...
                    className={`px-3 py-1.5 text-xs font-medium rounded-full transition-all border whitespace-nowrap ${activePeriod === period
